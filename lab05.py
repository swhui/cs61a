...
def distance(city1, city2):
    ...
    5.0
    """
    return sqrt((get_lat(city1) - get_lat(city2))**2 + (get_lon(city1) - get_lon(city2))**2)/1
...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    compare_city = make_city("Comparecity",lat, lon)
    if abs(distance(compare_city, city1)) < abs(distance(compare_city, city2)):
...
def sprout_leaves(t, values):
    ...
    if is_leaf(t):
        for leaf in values:
            other_branch += [tree(leaf)]
        return tree(label(t), other_branch)
    else:
        for other_leaf in branches(t):
            other_branch2 += [sprout_leaves(other_leaf, values)]
        return tree(label(t), other_branch2)
